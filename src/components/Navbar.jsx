...
const Navbar = () => {
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 rounded-lg text-white border-[#0E464F] border-2 shadow-md">
      ...
      <ul className="hidden md:flex gap-6">
        {links.map((link) => (
          <li key={link.id}>
            <Link
              to={link.url}
              className={`text-white hover:text-[#24A0B5] transition 
      ...
      {/* Ticket Button */}
      <button
        className="flex items-center gap-2 bg-white text-black px-4 py-2 rounded-lg hover:bg-[#24A0B5] hover:text-white transition cursor-pointer"
        onClick={() => navigate("/")}
      >
